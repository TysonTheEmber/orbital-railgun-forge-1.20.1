...
const float startTime = 4.0;
const float expansionTime = 32.0;
...
float sDist(vec3 p) {
    if (iTime < endTime) {
        float scale = 1.2 * (expansionTime * 25.0 / (localTime + 0.7) / 32.0 - 32.0 * (1.0 - pow(clamp(2.0 * localTime / expansionTime - 1.0, 0.0, 10.0), 2.0))) + 2.4;
        ...
        const float num = 6.28 / 6.0;
        float offset = -pow((localTime - 13.0) / 1.4, 2.0) + 60.0;
        ...
        float outer_spheres = min(length(p1) + max(scale, -3.0), length(p2) + max(scale, -3.0));

        float base = clamp(pow(localTime - 16.0, 3.0) * 50.0, 0.0, 5000.0);
        float height = min(pow(localTime - 11.0, 3.0) * 50.0, 5000.0);
    ...
    float explosion_cylindar = length(p.xz) + 8.0 / (localTime) - radius;
...
float shockwave(vec3 point) {
    ...
    if (iTime < endTime) {
        float speed_factor = 1.0 - pow(localTime / expansionTime, 2.0);
        speed_factor = clamp(speed_factor, -5.0, 5.0);
...
void main() {
    ...
    if (iTime < startTime) {
        ...
        float dist = max(
            ...
            -min(sdBox(end_point.xz, vec2(0.0, radius)), sdBox(end_point.xz, vec2(radius, 0.0)))
        );
    ...
    vec3 col = mix(blue, vec3(0.0), abs(sin(3.14 * localTime / expansionTime))) + vec3(smoothstep(5.0, 10.0, hit_result.y)) * blue;
    ...
    vec3 shockwave_color = mix(blue, vec3(1.0), clamp(iTime / endTime - 1.0, 0.0, 1.0));

    vec3 beamColor = mix(original * shockwave(end_point) * shockwave_color, vec3(col), threshold);
    vec4 color = vec4(beamColor, 1.0);
    color.rgb *= u_BeamColor;
    color.a *= u_BeamAlpha;
    fragColor = color;
}
